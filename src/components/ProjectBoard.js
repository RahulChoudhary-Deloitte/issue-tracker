...
const ProjectBoard = () => {
  ...
  useEffect(() => {
    const fetchAssignee = async () => {
      if (selectedProject && selectedProject.projectOwner && selectedProject.projectOwner.teamName) {
        const response = await projectApi.get(`/user?teamName=${selectedProject.projectOwner.teamName}`).catch((err) => {
          console.log(err);
        });
  ...
  useEffect(() => {
    if (selectedProject) {
      const fetchCardData = async () => {
        ...
        const cardDataWithSection = response.data.map((item) => ({
          ...item,
          section: "todo",
        }));
  ...
  function mapStatusToSectionName(status) {
    switch (status) {
      case "todo":
        return "todo";
      case "dev":
  ...
  const handleSelectProject = (value) => {
    const newSelectProject = projects.filter(project => project.projectID === value)
      ...
      {len < 1 ? (
        ...
        <>
          <div className="project-borad-header">
            ...
            <Link className="link" to={`/insights/${selectedProject.projectID}`}>
              VIEW INSIGHTS
            </Link>
          </div>
            ...
            <div className="select-project">
              <DropDown
                trigger={
                  <Button className="find-project">
                    <div>
                      <div>{selectedAssignee || "Select Assignee"}</div> <div>v</div>
                    </div>
                  </Button>
                }
                menuElementAttrs={{ className: "dropdown-menu" }}
            ...
            <div className="select-project">
              <DropDown
                trigger={
                  <Button className="find-project">
                    <div>
                      <div>{selectedPriority || "Select Priority"}</div> <div>v</div>
                    </div>
                  </Button>
                }
                menuElementAttrs={{ className: "dropdown-menu" }}
